...
const rootDirSize = rootDir.getSize();
const maxAvailableSpace = 70_000_000;
const requiredAvailableSpace = 30_000_000;

const unusedSpace = maxAvailableSpace - rootDirSize;
const neededFreedUpSpace = requiredAvailableSpace - unusedSpace;

const sizeToRemove = FileSystem.dirs.reduce((selectedSize, dir) => {
    const dirSize = dir.getSize();
    const shouldReplaceSelectedValue = dirSize > neededFreedUpSpace && dirSize < selectedSize;

    return shouldReplaceSelectedValue ? dirSize : selectedSize;
}, FileSystem.dirs[0].getSize());
